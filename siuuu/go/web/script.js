// API Configuration
const API_CONFIG = {
  baseURL: "http://localhost:8081", // Thay đổi theo Go server
  endpoints: {
    login: "/api/login",
    logout: "/api/logout",
    clients: "/api/clients",
...
//   } catch (error) {
//     console.error(`API call to ${endpoint} failed:`, error) // More specific error log
//     // Fallback to mock data in development
//     // Truyền currentUser vào getMockData để lọc log cho myDeviceLogs
...
async function apiCall(endpoint, options = {}) {
  ...
  try {
    ...
    if (response.status === 401) {
      // Xử lý tự động logout khi token hết hạn hoặc không hợp lệ
      localStorage.removeItem("authToken")
      localStorage.removeItem("currentUser")
      window.location.reload() // hoặc showPage("loginPage")
      return
    }
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`)
...
function updateStatsDisplay(stats) {
  ...
  document.querySelector(".online").textContent = `${stats.onlinePCs || 0} Online`
  document.querySelector(".offline").textContent = `${stats.offlinePCs || 0} Offline`
  document.getElementById("totalUsers").textContent = stats.totalUsers || 0
  document.querySelector(".active-users").textContent = `${stats.activeUsers || 0} Đang hoạt động`
  document.getElementById("totalAlerts").textContent = stats.totalAlerts || 0
  document.querySelector(".critical").textContent = `${stats.criticalAlerts || 0} Nghiêm trọng`
  document.querySelector(".warning").textContent = `${stats.warningAlerts || 0} Cảnh báo`
  // Removed systemPerformance display
}
...
async function loadDevices() {
  ...
  try {
    const clients = await apiCall(API_CONFIG.endpoints.clients)
    // Sort clients by agentID before updating the table
    clients.sort((a, b) => {
      const agentIdA = a.agentID.match(/\d+/)?.[0] || "0"
      const agentIdB = b.agentID.match(/\d+/)?.[0] || "0"
      return Number.parseInt(agentIdA) - Number.parseInt(agentIdB)
    })
    loadedClients = clients // Store the loaded clients
...
async function loadSystemLogs(agentId = "all") {
  ...
  console.log("Loading system logs for agent:", agentId) // Updated log message
  try {
    let url
    if (currentUser && currentUser.role !== "admin") {
    ...
    const logs = await apiCall(url) // Use the constructed URL
  ...
  try {
    ...
    if (response.success) {
      currentUser = {
        ...
        device: response.user.device, // Thêm thông tin thiết bị từ API response
      }
    ...
    if (users[username] && users[username].password === password) {
      currentUser = {
        ...
        password: users[username].password, // This is sensitive, but for mock it's fine
        role: users[username].role,
        fullName: users[username].fullName,
        email: users[username].email,
        device: users[username].device, // <--- Thêm dòng này để cập nhật device từ mock user
      }
      localStorage.setItem("currentUser", JSON.stringify(currentUser))
      showDashboard(currentUser.role)
      loginForm.reset()
...
async function loadUsers() {
  ...
  try {
    // Sử dụng endpoint mock cho đến khi có API thật
    const users = await apiCall(API_CONFIG.endpoints.getUsers)
    loadedUsers = users // Store users globally
...
async function getOTP(agentId) {
  ...
  try {
    ...
    if (response && response.otp) {
      // Use expiresTime from the response if available, otherwise fallback to expiresInSeconds
      const expiresIn = response.expiresTime !== undefined ? response.expiresTime : response.expiresInSeconds
      showOTPModal(response.otp, response.agentID, expiresIn)
      console.log(`OTP received for ${agentId}: ${response.otp}`)
    } else {
  ...
  try {
    ...
    if (addDeviceBtn) {
      addDeviceBtn.addEventListener("click", () => {
        ...
        document.getElementById("deviceId").value = "" // For Device Name (hostName)
        document.getElementById("deviceIp").value = ""
        document.getElementById("deviceHardwareId").value = ""
        document.getElementById("deviceAgentId").value = "" // For Agent ID
        document.getElementById("deviceUser").value = ""
        document.getElementById("modalTitle").textContent = "Thêm thiết bị mới"
        document.getElementById("deviceId").readOnly = false // Allow input for new device name
        document.getElementById("deviceAgentId").readOnly = false // Allow input for new agent ID
